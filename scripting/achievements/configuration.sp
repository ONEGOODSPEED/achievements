LoadAchivements()
{
	...
	do {
		...
		KvGetString(hKeyValues, "counter", SZF(sBuffer));
		if ( sBuffer[0] != 0 ) {
			SetTrieString(hAchievementData, "counter", sBuffer);
		}
		
		SetTrieValue(hAchievementData, "count", KvGetNum(hKeyValues, "count", 666));
		SetTrieValue(hAchievementData, "silent", KvGetNum(hKeyValues, "silent"));
