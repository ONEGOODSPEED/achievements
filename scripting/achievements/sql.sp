...
public SQLT_OnLoadProgress(Handle:hOwner, Handle:hQuery, const String:sError[], any:iUserId)
{
	...
	// update all achivements if late loaded
	if ( g_bLateLoaded[iClient] ) {
		decl Handle:hBuffer, bool:bUpdate;
		hBuffer = CreateTrie();
		for ( new i = 0; i < g_iTotalAchievements; ++i ) {
			...
			if ( GetTrieValue(g_hTrie_ClientProgress[iClient], sName, iCount) ) {
				SetTrieValue(hBuffer, sName, iCount);
			}
		}
		...
		while ( SQL_FetchRow(hQuery) ) {
			...
			new iCachedCount;
			if ( GetTrieValue(hBuffer, sName, iCachedCount) ) {
				SetTrieValue(g_hTrie_ClientProgress[iClient], sName, iCount + iCachedCount);
...
{
	// if client is loading put in queue
	if ( g_iClientId[iClient] == 0 ) {
		g_bLateLoaded[iClient] = true;
		return;
	}
	...
	if ( bUpdate ) {
		...
		WritePackString(hDatapack, sQuery);
		SQL_TQuery(g_hSQLdb, SQLT_OnInsertProgress, sQuery, hDatapack);
	}
}
...
public SQLT_OnInsertProgress(Handle:hOwner, Handle:hQuery, const String:sError[], any:hDatapack)
{
	if ( !hQuery ) {
		ResetPack(hDatapack);
		decl String:sQuery[256];
		ReadPackString(hDatapack, SZF(sQuery));
		//LogError("SQLT_OnInsertProgress failure: \"%s\" %s", sError, sQuery);
	}
	CloseHandle(hDatapack);
}

public SQLT_OnUpdateProgress(Handle:hOwner, Handle:hQuery, const String:sError[], any:data)
{
	if ( !hQuery ) {
